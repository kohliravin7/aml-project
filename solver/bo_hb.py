...
class BayesianOptimization(BaseSolver):
    ...
    def __init__(self, objective_func, lower, upper,
                 acquisition_func, model, maximize_func, cs,
        ...
        self.max_SH_iter = int(np.log(min_budget/max_budget)/np.log(eta))
        self.budget = (self.max_SH_iter+1)*self.max_budget
    ...
    def run(self, num_iterations=10, X=None, y=None):
        ...
        if X is None and y is None:
            ...
            for i, x in enumerate(init):
                ...
                new_y = self.objective_func(x, self.max_budget, self.configspace)
        ...
        for it in range(self.init_points, num_iterations):
            ...
            k = it - self.init_points
            # Choose next point to evaluate

            s = int(np.arange(self.max_SH_iter + 1)[::-1][k % (self.max_SH_iter+1)])
            n = int(np.ceil(int(self.budget/self.max_budget/(s+1))*self.eta**s)) # initial number of configurations
            r = self.max_SH_iter*self.eta**(-s) # initial number of iterations to run configurations for

            for i in range(s+1):
                
                # Run each of the n_i configs for r_i iterations and keep best n_i/eta
                n_i = n*self.eta**(-i)
                r_i = np.ceil(r*self.eta**(i))
                if r_i<self.min_budget:
                    r_i = self.min_budget
                val_losses = []
                T = []
                for j in range(n_i):
                    ...
                    start_time = time.time()
                    new_y = self.objective_func(new_x, r_i, self.configspace)
                    self.time_func_evals.append(time.time() - start_time)                    
